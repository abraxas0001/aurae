...
router.get('/', (req, res) => {
  ...
  const countQuery = `
    ...
    ${whereClause}
  `;
  const { count: totalCount } = db.prepare(countQuery).get(...params);
  const totalPages = Math.ceil(totalCount / perPage);
  ...
  const recipesQuery = `
    ...
    LIMIT ? OFFSET ?
  `;
  const recipes = db.prepare(recipesQuery).all(...params, perPage, offset);
...
router.post('/create', requireAuth, (req, res) => {
  ...
  try {
    ...
    if (err.message && err.message.includes('UNIQUE constraint failed')) {
      errorMsg = 'A recipe with a similar name already exists. Try a slightly different title.';
    } else if (err.message && err.message.includes('CHECK constraint failed')) {
      errorMsg = 'Invalid value for difficulty. Please select Easy, Medium, or Advanced.';
...
router.get('/:slug', async (req, res) => {
  ...
  if (needsAI) {
    try {
      ...
      // Save back to database so we don't call AI again
      db.prepare(`
        UPDATE recipes SET description = ?, prep_time = ?, cook_time = ?, servings = ?, difficulty = ?, ingredients = ?, instructions = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `).run(recipe.description, recipe.prep_time, recipe.cook_time, recipe.servings, recipe.difficulty, recipe.ingredients, recipe.instructions, recipe.id);
    } catch (err) {
      console.error('AI fallback failed:', err.message);
    }
  }
  ...
  if (req.session.user) {
    const fav = db.prepare('SELECT id FROM favorites WHERE user_id = ? AND recipe_id = ?')
      .get(req.session.user.id, recipe.id);
    isFavorited = !!fav;
  }
  ...
  const related = db.prepare(`
    ...
    ORDER BY RANDOM()
    LIMIT 3
  `).all(recipe.category_id, recipe.id);
